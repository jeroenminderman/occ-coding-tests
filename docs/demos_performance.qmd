...
```{python}
summary1.style.set_caption("occupationcoder-international 'accuracy', estimated as match rates between manually assigned occupation codes and either the highest ranked match ('Match to 1'), or as a match to any of the best 3 matches ('Match in 1-3'), grouped by either all examples or examples split by type.") \
        .hide(axis="index") \
...
- When considering *all* of the example data **in `{python} matches(example_coded, preds1, "prop")[0]` of cases, the top prediction from occupationcoder-international agrees with a manually assigned code**. When interpreting a "match" as the manual code being included in the top 3 predictions, this increases to `{python} matches(example_coded, preds1, "prop")[1]` cases. It is worth stressing this includes all example cases we *a priori* expected to be challenging.  
- As expected, direct match rates are much lower for known ambiguous cases (`{python} matches(t3, preds1, "prop")[0]`); but this is much improved by also considering the top 3 predictions (`{python} matches(t3, preds1, "prop")[1]`).
...
The ISCO-08 scheme consists of separate columns containing the ISCO code, the corresponding title, and various description fields. We here chose to simply concatenate the latter, resulting in a raw input file with two columns: ISCO code, and all description for that code. For the sake of simplicity, we provide a [helper function](https://github.com/jeroenminderman/occ-coding-tests/blob/a0bc505a45ade952bba7a2928013a21fd6bf4909/src/utils.py#L50) that does this; used as follows:
```{python, echo: true}
#| echo: true
...
Now that we have the scheme data processed in a format that can be used easily by Classifai, we download our chosen model for this example ([all-mpnet-base-v2](https://huggingface.co/sentence-transformers/all-mpnet-base-v2)). Note that for this to work you will need a working API key installed on your system.
...
```{python}
summary2.style.set_caption("Classifai 'accuracy', estimated as match rates between manually assigned occupation codes and either the highest ranked match ('Match to 1'), or as a match to any of the best 3 matches ('Match in 1-3'), grouped by either all examples or examples split by type.") \
        .hide(axis="index") \
        .set_properties(**{"text-align": "left"})
```

- Across *all* of the example data **in `{python} matches(example_coded, preds2, "prop")[0]` of cases, the top prediction from Classifai agrees with a manually assigned code**. When interpreting a "match" as the manual code being included in the top 3 predictions, this increases to `{python} matches(example_coded, preds2, "prop")[1]` cases. It is worth stressing this includes all example cases we *a priori* expected to be challenging.  
- Match rates to the top predictions are lower for known ambiguous cases (`{python} matches(t3a, preds2, "prop")[0]`); but this is much improved by also considering the top 3 predictions (`{python} matches(t3a, preds2, "prop")[1]`).

# Summary & comparison

Based on a [synthetic benchmark data set](example_data.qmd) (N = `{python} len(example_coded)`), and ***defining "accuracy" as a direct match between a preferred manually assigned code, and the best predicted match***;
  
- **Accuracy for occupationcoder-international is `{python} matches(example_coded, preds1, "prop")[0]`** across the full example data set (i.e. including known ambigous cases).  
- **Accuracy for Classifai is `{python} matches(example_coded, preds2, "prop")[0]`** across the same full example data set.  
- **Both tools perform better when all top 3 predictions are considered, with occupationcoder-international at `{python} matches(example_coded, preds1, "prop")[1]` and Classifai at `{python} matches(example_coded, preds2, "prop")[1]`**.
- Comparing occupationcoder-international and Classifai:  
    - **Classifai's accuracy performs better for *a priori* ambiguous cases (`{python} matches(t3a, preds2, "prop")[0]`), compared to occupationcoder-international (`{python} matches(t3, preds1, "prop")[0]`)**.
