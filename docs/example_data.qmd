...
## Background and context of example data

This repository includes a data file containing example text descriptions of occupations (including tasks, industry etc), coded to the ISCO-08 International Standard Classification of Occupations scheme.  
...
### Typical context of occupation data collection & coding

Data on respondents' occupations are typically collected in national surveys including e.g. national Labour Force Surveys or Censuses. In such surveys enumerators or field workers record the answers to questions like "*How would you describe the job you have done in the last X months*" and "*What tasks did this job involve*". The answer to these questions is typically recorded as short text strings (often not more than a few words).  

On the basis of these text inputs, either the same enumerators, or clerical coders following receipt of the data, are expected to assign at least one suitable code from the coding scheme used (here, ISCO-08). Such coding schemes are set up to provide classification of job descriptions to a standardised scheme, allowing consistent statistical reporting. ISCO-08 is the international standard for this, but many countries use bespoke coding schemes that are partly or wholly based on ISCO.
...
### Example data included here

In the example data, based on a combination of the job title, job description and industry description, at least one ISCO code is assigned as accurately as possible, and where relevant, a comment has been included.  
...
### Why use synthetic benchmark data?
...
While in an ideal world, this would use "real world" census data that has been manually coded by clerical coders, we are not aware of such a data set that is both publicly accessible and has been validated thoroughly (i.e. cross-coded by different coders, and revised as needed). Even if available using such real-world data does not allow for any control over, or analysis of, different potential issues with the input data: e.g. flagging of ambiguous cases as set out below.  
...
**We stress that, despite the advantages over real-world data listed above, this synthetic data approach does have limitations, and we encourage review of and feedback of the example data, particularly in terms of how it reflect real-world samples.**
...
> **Because these data are used to test and compare coding approaches, it is vital such data (1) adhere to the core purpose/aims specified above; and (2) formatted correctly.  
...
The latter type is to allow ability to distinguish between issues due to manual coding (enumerator/clerical coder mistakes), and failure of a coding tool to select the correct code, given such an expected match. It is worth stressing that in such cases, any discrepancy between the manual code given and the one(s) suggested by a coding tool could be due to *either* a mistake on the part of the enumerator, or a failure of the coding tool. In testing and validation of different tools, these TYPE classes provide a potential route to disambiguate such cases.
