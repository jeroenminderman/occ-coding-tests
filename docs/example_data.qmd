...
This repository includes a synthetic data file containing example text descriptions of occupations (including tasks, industry etc), coded to the [ISCO-08 International Standard Classification of Occupations scheme](https://ilostat.ilo.org/methods/concepts-and-definitions/classification-occupation/).  
...
To generate these synthetic data, job titles and descriptions were compiled from a variety of examples - including e.g. job adverts, descriptions of jobs/roles given as examples by third parties, and ones based on specific examples we have seen in typical census or survey data from a range of (English-speaking) countries including the UK, Namibia and Zimbabwe. Note that for data confidentiality and protection reasons, none of the examples included are directly taken from such data.  

These example cases were then coded "by hand" through referencing the ISCO-08 coding scheme - particularly focusing on the explicitly listed "job included here" records, the title of the code classes, and in particular their extended description of tasks involved. This coding step involves a degree of interpretation and semantic/contextual understanding. For example, a case with description "senior dev at gaming company" using wider understanding of the scheme implies "Software Developer" as an appropriate code. While the example data structure allows for the specification of up to 3 potential codes, the current version of the data predominantly has a single manual code assigned.  

**Although every effort was made to generate "fair" example data that reflects the kind of real-world cases we have seen, as of the time of writing the example data have been provided and coded by a single coder. Thus, these example data have limitations (see below) and both [contributions to and review of this data set are welcome and encouraged](#contributing-examples).**  
