...
Traditionally, appropriate classification of job descriptions to codes/classes relies on appropriate application of the coding scheme by enumerators or clerical coders; both in terms of their knowledge of its content and coverage, as well as its limitations.  

This heavily relies on appropriate training and maintenance of skills, and while not a trivial task, this is often underresourced. This can lead to data quality challenges. For example, it is vital that field enumerators ask follow up questions if initial answers are too limited in detail to allow accurate coding to a class in the scheme. 
...
Conistency is key, but also the main challenge. In many cases, even when appropriately trained, different coders may code inputs differently when they are  ambiguous or when the scheme is lacking in detail.  
...
It is worth stressing that here, we argue that such tools should always be viewed as "coding-assist tools", as opposed to "automatic coders". From the discussion presented throughout these pages it should be clear that (given limited input data) much of the ambiguity faced by human coders remains just as ambiguous to pretty much any coding tool. Some automation tools will struggle more than others, and keeping a "human in the loop" is vital to ensure data quality, particularly where the end product is business critical.  
...
[occupationcoder-international](https://github.com/datasciencecampus/occupationcoder-international) is a TF-IDF "fuzzy matching" tool, extending an existing tool for the UK SOC scheme to the international ISCO one.  

In brief, occupationcoder-international compares a given text input (e.g. XXXX) to each of the class descriptions in the ISCO scheme, and identifies which are most closely matched from a text-similarity perspective, suggesting the closest matches as the most appropriate class.  
...
While this may represent a significant advantage, it should be noted that the use of such pre-trained models can be more computationally intensive (but, depending on implementation, this can be managed), and access to them may be restricted in certain circumstances.
